...
def scrape_gojek_reviews(count=8000):
    result, _ = reviews(
        'com.gojek.app',
        lang='id',
        country='id',
...
def main():
    ...
    scrape_new = st.sidebar.checkbox("Ambil data baru dari Play Store", value=False)
    ...
    with tab1:
        ...
        if scrape_new:
            with st.spinner("Sedang mengambil data dari Play Store..."):
                try:
                    data = scrape_gojek_reviews(sample_size)
        ...
        if st.button("Muat Data yang Tersimpan"):
            try:
                ...
                st.warning("File data tidak ditemukan. Silakan ambil data baru terlebih dahulu.")
        ...
        if 'data' in locals():
            ...
            fig, ax = plt.subplots()
            sns.countplot(x='score', data=data, ax=ax)
            st.pyplot(fig)
